...
namespace FirMath
{
    public static class GameMath
    {
        ...
        public static string FormatNumber(this BigInteger number)
        {
            if (number == 0) return "0";
    
            string[] suffixes = { "", "K", "M", "B", "T", "Qa", "Qi", "Sx", "Sp", "Oc", "No", "Dc" };
    
            double log10 = BigInteger.Log10(number);
            int exponent = (int)Math.Floor(log10);
    
            if (exponent < 3) return number.ToString();
    
            int suffixIndex = exponent / 3;
            int adjustedExponent = suffixIndex * 3;
    
            double mantissa = Math.Pow(10, log10 - adjustedExponent);
            ...
            if (suffixIndex < suffixes.Length)
                ...
                return $"{mantissa}e{adjustedExponent}";
        }
    }
}